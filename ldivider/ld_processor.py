...
from ldivider.ld_convertor import (
    skimage_rgb2lab,
    df2rgba,
    rgba2df,
    hsv2df,
...
def calc_ciede(mean_list, cls_list):
    ...
    for i in range(len(mean_list)):
        ...
        for j in range(len(mean_list)):
            ...
            ciede = color.deltaE_ciede2000(
                skimage_rgb2lab(img_1), skimage_rgb2lab(img_2)
            )[0][0]
            ciede_list.append(ciede)
            cls_no.append(cls_list[i])
            tgt_no.append(cls_list[j])
    ciede_df = pd.DataFrame(
        {"cls_no": cls_no, "tgt_no": tgt_no, "ciede2000": ciede_list}
    )
    return ciede_df
...
def get_cls_update(ciede_df, df, threshold):
    set_list = [
        frozenset({cls, tgt})
        for cls, tgt in ciede_df[ciede_df["ciede2000"] < threshold][
            ["cls_no", "tgt_no"]
        ].to_numpy()
    ]
    merge_set = []
...
def get_update_df(df, merge_dict, mean_list, cls_list):
    update_df = df.copy()
    update_df["label"] = update_df["label"].apply(
        lambda x: x if x not in merge_dict.keys() else merge_dict[x]
    )
...
def get_seg_base(input_image, masks, th):
    ...
    for idx, mask in tqdm(enumerate(masks)):
        ...
        df = df.merge(
            mask_df, left_on=["x_l", "y_l"], right_on=["x_l_m", "y_l_m"], how="inner"
        )
...
def get_composite_layer(input_image, df):
    ...
    org_df["shadow_flg"] = org_df.apply(
        lambda x: (
            True
            if x["diff_r"] >= 0 and x["diff_g"] >= 0 and x["diff_b"] >= 0
            else False
        ),
        axis=1,
    )
    org_df["screen_flg"] = org_df.apply(
        lambda x: (
            True if x["diff_r"] < 0 and x["diff_g"] < 0 and x["diff_b"] < 0 else False
        ),
        axis=1,
    )

    shadow_df = org_df.copy()
    shadow_df["a"] = org_df.apply(
        lambda x: 255 if x["shadow_flg"] == True else 0, axis=1
    )
    ...
    addition_df["a"] = addition_df.apply(
        lambda x: 255 if x["screen_flg"] == False and x["shadow_flg"] == False else 0,
        axis=1,
    )
    ...
    subtract_df["a"] = subtract_df.apply(
        lambda x: 255 if x["screen_flg"] == False and x["shadow_flg"] == False else 0,
        axis=1,
    )
